...
### Added
- Integrated **Text Summarizer & Key Bullet Point Generator** in `src/services/summarizerService.ts` and `src/components/TextSummarizer.svelte`.
- Added term frequency scoring (tf-idf) with sentence position weighting for lead sentence extraction.
- Added sentence count presets (`1 Sentence`, `3 Sentences`, `5 Sentences`).
- Added compression ratio percentage metrics and 1-Click `📋 Copy Summary` action button.
- Added unit tests in `src/services/summarizerService.spec.ts`.
